...
function prepareRmzPrintLayout() {
  if (typeof ReportEngagementLayout === "undefined") {
    return () => {};
  }
...
const LAST_PPTX_IMPORT_STORAGE_KEY = "rmzCorpLastPptxImport";
...
function parseRuleSeverityCsvRows(csvText) {
  ...
  return dataRows
    ...
    .filter((r) => r.rule && r.high + r.medium + r.low > 0);
}

function formatRuleSeverityYLabel(label, maxLineLen) {
  const text = String(label || "").trim();
  const limit = Math.max(18, maxLineLen || 30);
  if (text.length <= limit) return text;
  const words = text.split(/\s+/);
  let line1 = "";
  let line2 = "";
  words.forEach((word) => {
    const next1 = line1 ? `${line1} ${word}` : word;
    if (!line2 && next1.length <= limit) {
      line1 = next1;
      return;
    }
    const next2 = line2 ? `${line2} ${word}` : word;
    line2 = next2.length <= limit ? next2 : `${next2.slice(0, limit - 3)}...`;
  });
  return line2 ? `${line1}\n${line2}` : `${line1.slice(0, limit - 3)}...`;
}

function fitRuleSeverityChartWrap() {
  const slide = document.querySelector("#slideRuleSeverity");
  const wrap = slide && slide.querySelector(".rule-severity-chart-wrap");
  if (!slide || !wrap) return 320;
  const titleEl = slide.querySelector(".rule-severity-title");
  const footerEl = slide.querySelector(".page-footer-bar");
  const titleH = titleEl ? titleEl.offsetHeight + 6 : 0;
  const footerH = footerEl ? footerEl.offsetHeight : 12;
  const avail = Math.max(250, slide.clientHeight - titleH - footerH - 16);
  wrap.style.height = `${avail}px`;
  wrap.style.maxHeight = `${avail}px`;
  return avail;
}

function syncRuleSeveritySlideLayout() {
  fitRuleSeverityChartWrap();
...
function parseEpsEventsRows(csvText) {
  ...
  const hasHeader = head.some(
    (k) =>
      ...
      k.includes("eventtype") ||
      k.includes("matchedevents") ||
      k.includes("countmatched")
  );
  const dataRows = hasHeader ? rows.slice(1) : rows;
  const findCol = (keys, fallback) => {
    if (!hasHeader) return fallback;
    for (const key of keys) {
      const idx = head.findIndex((k) => k === key || k.includes(key));
      if (idx >= 0) return idx;
    }
    return fallback;
  };
  const iSno = findCol(["sno"], 0);
  const iDevice = findCol(["reportingdevice"], 1);
  const iType = findCol(["eventtype"], 2);
  const iName = findCol(["eventname"], 3);
  const iCount = findCol(["countmatchedevents", "matchedevents", "countmatched"], 4);
  return dataRows
    .map((r, idx) => ({
...
function getEpsEventsLayoutOptions() {
  const maxRows = parseInt(getValue("epsEventsMaxRows"), 10);
  const fontPx = parseFloat(getValue("epsEventsTableFontSize"));
  return {
    maxRows: Number.isFinite(maxRows) && maxRows > 0 ? maxRows : 12,
    tableFontSizePx: Number.isFinite(fontPx) && fontPx > 0 ? fontPx : 0
  };
}

function initEpsEventsTableControls() {
  const reflow = (event) => {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    reflowEpsEventsTable();
  };
  ["epsEventsMaxRows", "epsEventsTableFontSize"].forEach((id) => {
    const el = document.getElementById(id);
    if (!el || el.dataset.epsLayoutBound === "1") return;
    el.dataset.epsLayoutBound = "1";
    el.addEventListener("change", reflow);
    el.addEventListener("input", reflow);
  });
  const btn = document.getElementById("epsEventsReflowBtn");
  if (btn && btn.dataset.epsLayoutBound !== "1") {
    btn.dataset.epsLayoutBound = "1";
    btn.addEventListener("click", reflow);
  }
}

function resetEpsEventsTableColumnResize() {
  const table = document.querySelector("#slideEpsEvents .eps-events-table");
  if (!table) return;
  delete table.dataset.riskResizeBound;
  table.querySelectorAll("thead th .resizer").forEach((el) => el.remove());
}

function getEpsEventsRowsForExport() {
  const { maxRows } = getEpsEventsLayoutOptions();
  return parseEpsEventsRows(getValue("epsEventsCsv")).slice(0, maxRows);
}

function reflowEpsEventsTable() {
  const slide = document.getElementById("slideEpsEvents");
  resetEpsEventsTableColumnResize();
  renderEpsEventsTable();
  if (!slide) return;
  requestAnimationFrame(() => {
    syncEpsEventsSlideLayout();
    initEpsEventsTableColumnResize();
    slide.scrollIntoView({ block: "center", behavior: "smooth" });
  });
}

function initEpsEventsTableColumnResize() {
  if (typeof initRiskTableColumnResize !== "function") return;
  const slide = document.getElementById("slideEpsEvents");
  if (!slide) return;
  initRiskTableColumnResize(slide, {
    onColumnResizeEnd: () => syncEpsEventsSlideLayout()
  });
}

function getPptEpsEventsSlideLayout(dataRowCount, sy) {
  const footerTop = 4.95 * sy;
  const tableGap = 0.04 * sy;
  const tableY = 0.78 * sy;
  const tableH = Math.max(0.5 * sy, footerTop - tableY - tableGap);
  const rowCount = Math.max(1, dataRowCount) + 1;
  const rowH = tableH / rowCount;
  const layoutOpts = getEpsEventsLayoutOptions();
  let fontSize = rowCount >= 11 ? 6.5 : rowCount >= 10 ? 7 : rowCount >= 8 ? 7.5 : 8.2;
  if (layoutOpts.tableFontSizePx > 0) {
    fontSize = Math.min(12, Math.max(6, layoutOpts.tableFontSizePx * 0.64));
  }
  return {
    tableY,
    tableH,
    rowH,
    rowHs: Array(rowCount).fill(rowH),
    fontSize,
    footerTop
  };
}
...
function renderEpsEventsTable() {
  ...
  if (!body) return;
  const { maxRows } = getEpsEventsLayoutOptions();
  const rows = parseEpsEventsRows(getValue("epsEventsCsv")).slice(0, maxRows);
  if (!rows.length) {
...
function getEpsEventsWrapBudget(slide, wrap, rowCount) {
  const footer = slide.querySelector(".page-footer-bar");
  const gap = 8;
  const minRowPx = 15;
  const rows = Math.max(1, rowCount || 1);
  const minNeeded = rows * minRowPx + 34;

  if (wrap && footer) {
    const measured = footer.offsetTop - wrap.offsetTop - gap;
    if (Number.isFinite(measured) && measured >= minNeeded) return measured;
  }

  const title = slide.querySelector(".eps-events-title");
  const header = slide.querySelector(".page-header-right");
  const reservedTop =
    (header?.offsetHeight || 0) + (title?.offsetHeight || 0) + slidePaddingTop(slide) + 6;
  const footerH = footer?.offsetHeight || 42;
  const slideH = slide.clientHeight || 540;
  return Math.max(minNeeded, slideH - reservedTop - footerH - gap);
}
...
function syncEpsEventsSlideLayout() {
  ...
  const budget = getEpsEventsWrapBudget(slide, wrap, rowCount);

  wrap.style.height = `${budget}px`;
  wrap.style.maxHeight = `${budget}px`;
  ...
  const layoutOpts = getEpsEventsLayoutOptions();
  let fontRem =
    rowCount >= 10 ? 0.62 : rowCount >= 8 ? 0.7 : rowCount >= 6 ? 0.78 : 0.92;
  if (layoutOpts.tableFontSizePx > 0) {
    fontRem = layoutOpts.tableFontSizePx / 16;
  }
  const minFontRem = rowCount >= 10 ? 0.48 : 0.56;
  let padH = rowCount >= 10 ? 3 : 5;
  ...
  const distributeRowHeights = () => {
    ...
    const heights = Array(count).fill(Math.floor(bodyH / count));
    let remainder = bodyH - heights.reduce((sum, h) => sum + h, 0);
    for (let i = 0; i < remainder; i += 1) {
      heights[i % count] += 1;
    }
    tbodyRows.forEach((tr, idx) => {
      const h = heights[idx] || heights[0];
    ...
    return heights[0] || Math.floor(bodyH / count);
  };

  const applyFill = () => {
    applyBase();
    const theadH = thead ? thead.offsetHeight : 0;
    bodyH = Math.max(rowCount * 12, budget - theadH - 6);
    const sampleRowH = distributeRowHeights();
    const padV = Math.max(1, Math.min(4, Math.floor((sampleRowH - fontRem * 12) / 2)));
    table.querySelectorAll("thead th").forEach((cell) => {
      cell.style.paddingTop = `${Math.max(2, padV)}px`;
      cell.style.paddingBottom = `${Math.max(2, padV)}px`;
    });
  ...
  const allRowsVisible = () => {
    if (!footer || !tbodyRows.length) return true;
    const last = tbodyRows[tbodyRows.length - 1];
    return last.getBoundingClientRect().bottom <= footer.getBoundingClientRect().top - gap;
  };

  applyFill();
  for (let pass = 0; pass < 80; pass += 1) {
    const over = table.scrollHeight - budget;
    if (over <= 2 && !overflowsFooter() && allRowsVisible()) break;
    if (fontRem > minFontRem) {
      fontRem -= 0.025;
      padH = Math.max(2, padH - (pass % 4 === 0 ? 1 : 0));
      applyFill();
      continue;
    }
    if (over > 2 && bodyH > rowCount * 11) {
...
function getPptRiskSlideLayout(slideIndex, dataRowCount, sy) {
  ...
  if (hasNarrative) {
    ...
    const narrativeCap = dataRows >= 3 ? 0.82 : 1.15;
    narrativeH = Math.min(narrativeCap, 0.08 * lines + 0.16) * sy;
    narrativeY = titleY + titleH + gapIn;
  ...
  const rowH = tableH / rowCount;
  const rowHs = Array(rowCount).fill(rowH);
  const tableFontSize =
    dataRows >= 3 ? (hasNarrative ? 7.5 : 8) : dataRows === 2 ? 8.5 : 9;
...
function renderRuleSeverityChart() {
  ...
  const wrapH = fitRuleSeverityChartWrap();
  const chartFont = "Calibri, Segoe UI, Arial, sans-serif";
  const yFontSize = rowCount > 14 ? 9 : rowCount > 11 ? 10 : 11;
  const labelLineLen = rowCount > 14 ? 28 : rowCount > 11 ? 32 : 36;
  const maxVal = Math.max(...rows.map((r) => Math.max(r.high, r.medium, r.low)), 1);
  const axisMax = Math.max(6, Math.ceil((maxVal * 1.08) / 3) * 3);
  const xStep = axisMax <= 12 ? 1 : 3;
  const barThickness = Math.max(6, Math.min(11, Math.floor((wrapH - 92) / rowCount)));
  ensureTrendDataLabelsPlugin();
  const plugins = {
    title: {
      display: true,
      text: "Potential Alert - Severity Categories",
      font: { size: 14, weight: "bold", family: chartFont },
    ...
    legend: {
      ...
      position: "bottom",
      labels: {
        ...ReportCharts.buildSquareLegendLabels("#000"),
        boxWidth: 12,
        boxHeight: 12,
        padding: 12,
        font: { size: 11, weight: "bold", family: chartFont }
      }
    }
  };
      ...
      scales: {
        x: {
          ...
          ticks: {
            stepSize: xStep,
            color: "#000000",
            maxTicksLimit: Math.ceil(axisMax / xStep) + 1,
            font: { size: 11, weight: "bold", family: chartFont }
          }
        },
        y: {
          ...
          ticks: {
            ...
            mirror: false,
            padding: 2,
            font: { size: yFontSize, weight: "bold", family: chartFont },
            callback: function (value) {
              return formatRuleSeverityYLabel(this.getLabelForValue(value), labelLineLen);
            }
          }
        }
      }
...
function slaCustomerLabel(customerName) {
  return (customerName || "the customer").trim();
}
...
function getHiResChartDataUrl(canvas, chart, captureDpr) {
  ...
  const upscale = typeof chart === "number" ? chart : 2;
  ...
  if (upscale <= 1) return canvas.toDataURL("image/png");
...
async function exportPptxLegacy() {
  ...
  const riskCellOpts = { fontFace: "Calibri", valign: "middle", wrap: true };
  ...
  riskChunks.forEach((chunk, i) => {
    ...
    const riskFont =
      getRiskLayoutOptions().tableFontSizePx > 0
        ? Math.min(18, Math.max(7.5, getRiskLayoutOptions().tableFontSizePx * 0.64))
        : riskLayout.tableFontSize || 9;
    slide.addTable(riskTableData, {
      x: margin,
      y: riskLayout.tableY,
      w: contentW,
      h: riskLayout.tableH,
      rowH: riskLayout.rowHs || riskLayout.rowH,
      fontSize: riskFont,
  ...
  const ruleRowsPptx = parseRuleSeverityCsvRows(getValue("ruleSeverityCsv"));
  if (ruleRowsPptx.length) {
    slide = pptx.addSlide();
  ...
  if (responseCanvas) {
    slide.addImage({
      data: getHiResChartDataUrl(responseCanvas),
      x: 0.35 * sx,
      y: 1.0 * sy,
      w: 2.85 * sx,
      h: 1.75 * sy
  ...
  if (remediationCanvas) {
    slide.addImage({
      data: getHiResChartDataUrl(remediationCanvas),
      x: 0.35 * sx,
      y: 3.45 * sy,
      w: 2.85 * sx,
      h: 1.75 * sy
  ...
  const responseTbl = [
    ...
    [
      {
        text: "High Severity ticket resulted in extremely serious interruptions to Business system. It has affected the user community",
        options: { fill: { color: "EEF6F9" } }
      },
    ...
    [
      {
        text: "Low Severity A Request/Query/Service that does not change existing service structure and no cost implication",
        options: { fill: { color: "EEF6F9" } }
      },
  ...
  const remediationTbl = [
    ...
    [
      {
        text: "Low A Request/Query/Service that does not change existing service structure and no cost implication",
        options: { fill: { color: "EEF6F9" } }
      },
  ...
  renderEpsEventsTable();
  syncEpsEventsSlideLayout();
  const epsEventsRows = getEpsEventsRowsForExport();
  slide = pptx.addSlide();
  ...
  const epsCellOpts = { fontFace: "Calibri", valign: "middle", wrap: true };
  const epsHeadOpts = { bold: true, fill: { color: "8ED8DF" }, align: "center", ...epsCellOpts };
  const epsEventsTableData = [
    [
      { text: "S.No", options: epsHeadOpts },
      { text: "Reporting Device", options: epsHeadOpts },
      { text: "Event Type", options: epsHeadOpts },
      { text: "Event Name", options: epsHeadOpts },
      { text: "Matched Events", options: epsHeadOpts }
    ]
  ];
  if (epsEventsRows.length) {
    epsEventsRows.forEach((r, idx) => {
      const n = Number(String(r.matchedEvents).replace(/,/g, ""));
      epsEventsTableData.push([
        { text: String(r.sno || idx + 1), options: { align: "center", ...epsCellOpts } },
        { text: r.device, options: { align: "center", ...epsCellOpts } },
        { text: r.eventType, options: { align: "center", ...epsCellOpts } },
        { text: r.eventName, options: { align: "center", ...epsCellOpts } },
        {
          text: Number.isFinite(n) ? n.toLocaleString("en-US") : String(r.matchedEvents || ""),
          options: { align: "center", ...epsCellOpts }
        }
      ]);
    });
  } else {
    epsEventsTableData.push([
      { text: "-", options: epsCellOpts },
      { text: "No rows found. Upload EPS Events CSV.", options: epsCellOpts },
      { text: "", options: epsCellOpts },
      { text: "", options: epsCellOpts },
      { text: "", options: epsCellOpts }
    ]);
  }
  const epsLayout = getPptEpsEventsSlideLayout(epsEventsRows.length, sy);
  slide.addTable(epsEventsTableData, {
    x: 0.65 * sx,
    y: epsLayout.tableY,
    w: 8.7 * sx,
    h: epsLayout.tableH,
    fontSize: epsLayout.fontSize,
    rowH: epsLayout.rowHs,
    colW: [0.55 * sx, 1.45 * sx, 2.35 * sx, 2.45 * sx, 1.9 * sx],
  ...
  document.getElementById("printBtn").addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (typeof preparePrintSlideOrder === "function") {
        preparePrintSlideOrder("rmz");
      }
      finishRmzPrintLayout();
      reportPdfLayoutRestore = prepareRmzPrintLayout();
